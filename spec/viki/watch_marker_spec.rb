...
describe Viki::WatchMarker, api: true do
  it 'fetches list of watch markers' do
    ...
    watch_markers = nil
    described_class.fetch(user_id: '1u') { |res| watch_markers = res.value }
    Viki.run
  ...
  it 'fetches list with given timestamp' do
    stub_api 'users/1u/watch_markers.json', json_fixture(:watch_marker), api_version: 'v4'

    watch_markers = nil
    described_class.should_receive(:fetch).with(user_id: '1u', from: 1455950940)

    described_class.fetch(user_id: '1u', from: 1455950940) { |res| watch_markers = res.value }
  end 
end
